...
export const getAdminOverview = createServerFn().handler(async (): Promise<AdminOverview> => {
  const directory = env.KEWEKE_ALIASES.getByName(ALIAS_DIRECTORY_NAME);
  const [userIds, listIds] = await Promise.all([directory.listUserIds(), directory.listListIds()]);

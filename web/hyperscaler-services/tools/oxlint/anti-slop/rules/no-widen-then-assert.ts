...
type ConstantDeclarator = ESTree.VariableDeclarator & {
  readonly init: ESTree.Expression;
};

type IdentifierPattern = Extract<ESTree.VariableDeclarator["id"], { readonly type: "Identifier" }>;

function hasInitializedExpression(
  declarator: ESTree.VariableDeclarator,
): declarator is ConstantDeclarator {
  return declarator.init !== null;
}

function hasIdentifierPattern(
  declarator: ConstantDeclarator,
): declarator is ConstantDeclarator & { readonly id: IdentifierPattern } {
...
function isBroadRecordLiteral(type: ESTree.TSTypeLiteral): boolean {
  ...
  if (member?.type !== "TSIndexSignature" || member.parameters.length !== 1) {
    return false;
  }
...
function constantDeclarator(variable: Variable): ConstantDeclarator | null {
  const declarator = variableDeclarator(variable);
  if (declarator === null || declarator.parent.type !== "VariableDeclaration") {
    return null;
  }
  if (
    declarator.parent.kind !== "const" ||
    !hasInitializedExpression(declarator) ||
...
function constantIdentifierDeclarator(
  variable: Variable,
): (ConstantDeclarator & { readonly id: IdentifierPattern }) | null {
  const declarator = constantDeclarator(variable);
  if (declarator === null || !hasIdentifierPattern(declarator)) {
    return null;
  }
  return declarator;
}

const directEvidenceTypes = new Set([
  "ArrayExpression",
  "ArrowFunctionExpression",
  "ClassExpression",
  "FunctionExpression",
...
function directKnownValueEvidence(
  expression: ESTree.Expression,
  ...
  return directEvidenceTypes.has(expression.type) || expression.type === "ObjectExpression"
    ? { type: null }
    : undefined;
}
...
function knownIdentifierEvidence(
  ...
  const declarator = constantDeclarator(variable);
  if (declarator === null || functionBoundary(declarator) !== boundary) {
    return null;
  }
...
function knownValueEvidence(
  ...
  return unwrapped.type === "Identifier"
    ? knownIdentifierEvidence(unwrapped, scopes, boundary, visitedVariables)
    : null;
}
...
function widenedBinding(
  ...
  const declarator = constantIdentifierDeclarator(variable);
  if (declarator === null) {
    return null;
  }

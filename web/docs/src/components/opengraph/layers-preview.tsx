...
export function LayersPreview() {
  return (
    <PreviewShell label="Layers — live panel" caption="Search, reorder by dragging, toggle visibility/lock, double-click to rename. The + and ⋯ menus are the same dropdowns as in the editor.">
      <div className="mx-auto max-w-sm border bg-background">
        <div className="flex h-10 items-center justify-between border-b px-3">
          ...
          <div className="flex gap-0.5">
            <span className="flex size-6 items-center justify-center rounded-md text-muted-foreground">+</span>
            <span className="flex size-6 items-center justify-center rounded-md text-muted-foreground">⋯</span>
          </div>
        </div>
        <div className="border-b p-3">
          <div className="rounded-md border bg-input/20 px-2 py-1.5 text-xs text-muted-foreground">Search layers</div>
        </div>
        <div className="p-2">
          {mockLayers.map((l) => {
            ...
            return (
              ...
              >
                <span className="flex min-w-0 flex-1 items-center gap-1.5 px-1.5">
                  ...
                  <span className="ml-auto text-[10px] text-muted-foreground">{l.type === "text" ? "Text" : "Shape"}</span>
                </span>
...
export function ColorPickerPreview() {
  return (
    <PreviewShell label="Color picker" caption="Click any swatch to open the picker. The same picker appears in the toolbox and in Properties. Drag the field or type a hex.">
      <div className="flex flex-col gap-3 p-4">
        ...
        <div className="mx-auto w-56 rounded-xl border bg-popover p-3 shadow-lg">
          <div className="h-36 rounded-md" style={{ background: "linear-gradient(to right, #f00 0%, #ff0 17%, #0f0 33%, #0ff 50%, #00f 67%, #f0f 83%, #f00 100%)" }} />
          <div className="mt-3 flex gap-2">
            <div className="h-8 w-24 rounded-md border bg-white font-mono text-xs leading-8">#E91AE7</div>
